...
//            'package_length_type' => 'N',
    ...
    public function onTask($serv, $task_id, $from_id, $data) {
        ...
        $this->notifyAlarm($saveData['identity_id']);
    ...
    public function saveData(array $arr) {
        $collection = $this->getDB()->selectCollection('puhai_home_report');
...
//        $data = bin2hex($data); 
    ...
    public function analyzeBinInfo($statu_bit, $sub_status_bit) {
        ...
        if (isset($this->analyzeData['cmd']['status_bit']['bit3'][$statusBin[3]]['sub_status_bit']['bit2'][$subStatusBin[2]])) {
            $info['alarm_co'] = $this->analyzeData['cmd']['status_bit']['bit3'][$statusBin[3]]['sub_status_bit']['bit2'][$subStatusBin[2]];
        }
        if (isset($this->analyzeData['cmd']['status_bit']['bit3'][$statusBin[3]]['sub_status_bit']['bit4'][$subStatusBin[4]])) {
            $info['alarm_emperature'] = $this->analyzeData['cmd']['status_bit']['bit3'][$statusBin[3]]['sub_status_bit']['bit4'][$subStatusBin[4]];
        }

        return $info;
    }

    public function notifyAlarm($identity_id) {
        $client = new swoole_client(SWOOLE_SOCK_TCP);
        if (!$client->connect('0.0.0.0', 9529, -1)) {
